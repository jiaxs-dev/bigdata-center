    ...
    <el-row>
      <el-col :span="24" style="padding-bottom: 20px">
        <el-card>
          ...
          <el-table :data="table.tableData" stripe style="width: 100%" @sort-change="sortTable">
...
<script>
  import {
    ...
    LineChartDimensions,
    pickerOptions
  } from "./advertising"; //引入广告分析页面常量
  import {getTheMomentFewDaysAgo} from "../../../assets/js/dateFun"; //引入日期处理函数
  import {advertisingLineChartApi, advertisingTableApi, advertisingDimensions} from '../../../api/analysis/analysis'; //引入调用后台api接口
  ...
  export default {
    ...
    data() {
      return {
        ...
        table: {
          ...
          SelectedDynamicTableColumns: ['poster_name'],
    ...
    methods: {
      ...
      dimensionValueQuery(index, dimension) {
        ...
        let dis = diss[index];
        advertisingDimensions({dimension: dimension}).then(res => {
          if (res.code == 0) {
      ...
      getTableData() {
        ...
        advertisingTableApi(param).then(res => {
          if (res.code == 0) {
            if (this.table.real) {
      ...
      lineChartQuery() {
        ...
        advertisingLineChartApi(param).then(res => {
          if (res.code == 0) {
            let chartData = [];
            if (this.lineChart.multi) {
