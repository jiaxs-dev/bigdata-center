  ...
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px"
      ...
      <el-form-item prop="username">
        <el-input type="text" v-model="ruleForm.username" auto-complete="off" id="username"
      ...
      <el-form-item prop="yanzhengma">
        <div class="check-code-wrapper">
          <div class="yanzhengma-wrapper">
            <el-input v-model="ruleForm.yanzhengma" placeholder="请输入验证码">
...
<script type="text/ecmascript-6">
  ...
  import {getCookie, removeCookie, setCookieForever} from "../../assets/js/commonFun";
        ...
        this.$refs[formName].validate(valid => {
          if (valid) {
            if (this.rememberpwd == true) {
              ...
              //传入账号名，密码，和保存天数3个参数
              setCookieForever('user',this.ruleForm.username);
              setCookieForever('pwd',this.ruleForm.password);
            }else{
              removeCookie('user');
              removeCookie('pwd');
            }
            loginApi(this.ruleForm).then(res => {
              if (res.code == 0) {
                this.login({userName: this.ruleForm.username, token: this.ruleForm.username}); //actions
                this.logining = false;
            ...
            //this.$store.commit("login/login", {userName: this.ruleForm.userName, token: res.result}) mutations
            //this.$store.dispatch("login/login", {userName: this.ruleForm.userName, token: res.result}); actions
...
<style lang="scss" scoped>
  .login-wrap {
    ...
    /*background: #f0f2f5 url('../../assets/img/login/TVYTbAXWheQpRcWDaDMu.svg') no-repeat center 110px;*/
